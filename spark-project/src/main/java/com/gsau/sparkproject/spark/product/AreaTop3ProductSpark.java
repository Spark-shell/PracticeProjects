...
import com.gsau.sparkproject.domain.AreaTop3Product;
...
public class AreaTop3ProductSpark {
    ...
    public static void main(String[] args) {
        SparkConf conf = new SparkConf().setAppName("AreaTop3ProductSpark");                                           // 创建SparkConf
        SparkUtils.setMaster(conf);
        JavaSparkContext sc = new JavaSparkContext(conf);                                                              // 构建Spark上下文
        SQLContext sqlContext = SparkUtils.getSQLContext(sc.sc());
//		sqlContext.setConf("spark.sql.shuffle.partitions", "1000"); 
//		sqlContext.setConf("spark.sql.autoBroadcastJoinThreshold", "20971520");

        sqlContext.udf().register("concat_long_string", new ConcatLongStringUDF(), DataTypes.StringType);       // 注册自定义函数
        sqlContext.udf().register("get_json_object", new GetJsonObjectUDF(), DataTypes.StringType);             // 注册自定义函数
        sqlContext.udf().register("random_prefix", new RandomPrefixUDF(), DataTypes.StringType);                // 注册自定义函数
        sqlContext.udf().register("remove_random_prefix", new RemoveRandomPrefixUDF(), DataTypes.StringType);   // 注册自定义函数
        sqlContext.udf().register("group_concat_distinct", new GroupConcatDistinctUDAF());

        SparkUtils.mockData(sc, sqlContext);                                                                           // 准备模拟数据
        ITaskDAO taskDAO = DAOFactory.getTaskDAO();                                                                    // 获取命令行传入的taskid，查询对应的任务参数
        ...
        String startDate = ParamUtils.getParam(taskParam, Constants.PARAM_START_DATE);                                 //开始时间
        String endDate = ParamUtils.getParam(taskParam, Constants.PARAM_END_DATE);                                     //结束时间
        //cityid2clickActionRDD==><cityid,row>
        JavaPairRDD<Long, Row> cityid2clickActionRDD = getcityid2ClickActionRDDByDate(sqlContext, startDate, endDate); // 查询用户指定日期范围内的点击行为数据（city_id，在哪个城市发生的点击行为）
        System.out.println("cityid2clickActionRDD: " + cityid2clickActionRDD.count());

        JavaPairRDD<Long, Row> cityid2cityInfoRDD = getcityid2CityInfoRDD(sqlContext);                                 // 从MySQL中查询城市信息       技术点2：异构数据源之MySQL的使用
        System.out.println("cityid2cityInfoRDD: " + cityid2cityInfoRDD.count());

        generateTempClickProductBasicTable(sqlContext, cityid2clickActionRDD, cityid2cityInfoRDD);                     // 生成点击商品基础信息临时表  技术点3：将RDD转换为DataFrame，并注册临时表
        generateTempAreaPrdocutClickCountTable(sqlContext);                                                            // 生成各区域各商品点击次数的临时表
        generateTempAreaFullProductClickCountTable(sqlContext);                                                        // 生成包含完整商品信息的各区域各商品点击次数的临时表

        JavaRDD<Row> areaTop3ProductRDD = getAreaTop3ProductRDD(sqlContext);                                           // 使用开窗函数获取各个区域内点击次数排名前3的热门商品
        System.out.println("areaTop3ProductRDD: " + areaTop3ProductRDD.count());
        // 因为实际上，就这个业务需求而言，计算出来的最终数据量是比较小的, 所以可以直接将数据collect()到本地,用批量插入的方式，一次性插入mysql即可
    ...
    private static JavaPairRDD<Long, Row> getcityid2ClickActionRDDByDate(
        ...
        JavaPairRDD<Long, Row> cityid2clickActionRDD = clickActionRDD.mapToPair(                                       //cityid2clickActionRDD==><cityid,row>
    ...
    /**
     * @Description: 使用Spark SQL从MySQL中查询城市信息
     * @author: wangguoqing
    ...
    private static JavaPairRDD<Long, Row> getcityid2CityInfoRDD(SQLContext sqlContext) {
        ...
        DataFrame cityInfoDF = sqlContext.read().format("jdbc").options(options).load();                               // 通过SQLContext去从MySQL中查询数据
        JavaRDD<Row> cityInfoRDD = cityInfoDF.javaRDD();

        JavaPairRDD<Long, Row> cityid2cityInfoRDD = cityInfoRDD.mapToPair(                                             // 返回RDD
                new PairFunction<Row, Long, Row>() {
                    private static final long serialVersionUID = 1L;
    ...
    private static void generateTempClickProductBasicTable(SQLContext sqlContext, JavaPairRDD<Long, Row> cityid2clickActionRDD, JavaPairRDD<Long, Row> cityid2cityInfoRDD) {
        JavaPairRDD<Long, Tuple2<Row, Row>> joinedRDD = cityid2clickActionRDD.join(cityid2cityInfoRDD);                // 执行join操作，进行点击行为数据和城市数据的关联
        JavaRDD<Row> mappedRDD = joinedRDD.map(                                                                        // 将上面的JavaPairRDD，转换成一个JavaRDD<Row>（才能将RDD转换为DataFrame）
                new Function<Tuple2<Long, Tuple2<Row, Row>>, Row>() {
                    ...
                    public Row call(Tuple2<Long, Tuple2<Row, Row>> tuple)
                        ...
                        return RowFactory.create(cityid, cityName, area, productid);                                   //row对象，cityid,cityName,area,productid
                    }
                });
        List<StructField> structFields = new ArrayList<StructField>();                                                           // 基于JavaRDD<Row>的格式，就可以将其转换为DataFrame
        structFields.add(DataTypes.createStructField("city_id", DataTypes.LongType, true));            //构建字段的类别
        ...
        StructType schema = DataTypes.createStructType(structFields);                                                  //创建DataFrame的字段类别对象
        DataFrame df = sqlContext.createDataFrame(mappedRDD, schema);                                                  //创建根据RDD创建DataFrame
        System.out.println("tmp_click_product_basic: " + df.count());
        df.registerTempTable("tmp_click_product_basic");                                                   // 将DataFrame中的数据，注册成临时表（tmp_click_product_basic）
    }
    ...
    private static void generateTempAreaPrdocutClickCountTable(
        ...
        // 按照area和product_id两个字段进行分组,计算出各区域各商品的点击次数, 可以获取到每个area下的每个product_id的城市信息拼接起来的串
        String sql =
                "SELECT "
                        + "area,"
                        + "product_id,"
...
//				+ "GROUP BY product_id_area ";  
        ...
        DataFrame df = sqlContext.sql(sql);                                                                            // 使用Spark SQL执行这条SQL语句
        System.out.println("tmp_area_product_click_count: " + df.count());
        df.registerTempTable("tmp_area_product_click_count");                                              // 再次将查询出来的数据注册为一个临时表,各区域各商品的点击次数（以及额外的城市列表）
    }
