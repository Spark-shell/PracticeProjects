...
public class AdClickRealTimeStatSpark {
	...
	public static void main(String[] args) {
		SparkConf conf = new SparkConf()                                                      						   // 构建Spark Streaming上下文
				.setMaster("local[2]")
				.setAppName("AdClickRealTimeStatSpark");
//				.set("spark.serializer", "org.apache.spark.serializer.KryoSerializer");
//				.set("spark.default.parallelism", "1000");
//				.set("spark.streaming.blockInterval", "50");                                                           //spark streaming，每隔一小段时间，会去收集一次数据源（kafka）中的数据，做成一个batch,每次都是处理一个batch中的数据
//				.set("spark.streaming.receiver.writeAheadLog.enable", "true");   

		JavaStreamingContext jssc = new JavaStreamingContext(conf, Durations.seconds(5));                           //spark streaming的上下文是构建JavaStreamingContext对象
		...
		Map<String, String> kafkaParams = new HashMap<String, String>();                                               //主要要放置的就是，你要连接的kafka集群的地址（broker集群的地址列表）
		kafkaParams.put("metadata.broker.list", ConfigurationManager.getProperty(Constants.KAFKA_METADATA_BROKER_LIST));
		
		String kafkaTopics = ConfigurationManager.getProperty(Constants.KAFKA_TOPICS);                                 //构建topic set
		...
		// 基于kafka direct api模式，构建出了针对kafka集群中指定topic的输入DStream，两个值，val1，val2；val1没有什么特殊的意义；val2中包含了kafka topic中的一条一条的实时日志数据
		JavaPairInputDStream<String, String> adRealTimeLogDStream = KafkaUtils.createDirectStream(                     //创建Kafka到Spark的数据流
				jssc, 
				String.class,                        //返回的数据类型：value1
				String.class, 					     //返回的数据类型：value2
				StringDecoder.class,                 //Value1的解码类型
				StringDecoder.class,                 //Value2的解码类型，Value2中包含了当前需要的数据（内容是：timestamp_province_city_userid_adid）
				kafkaParams,
				topics);
//		adRealTimeLogDStream.repartition(1000);
		JavaPairDStream<String, String> filteredAdRealTimeLogDStream = filterByBlacklist(adRealTimeLogDStream);        // 根据动态黑名单进行数据过滤
		generateDynamicBlacklist(filteredAdRealTimeLogDStream);                                                        // 生成动态黑名单
		
		JavaPairDStream<String, Long> adRealTimeStatDStream = calculateRealTimeStat(filteredAdRealTimeLogDStream);     // 业务功能一：计算广告点击流量实时统计结果（yyyyMMdd_province_city_adid,clickCount），最粗
		calculateProvinceTop3Ad(adRealTimeStatDStream);                                                                // 业务功能二：实时统计每天每个省份top3热门广告,统计的稍微细一些了
		calculateAdClickCountByWindow(adRealTimeLogDStream);                                                           // 业务功能三：实时统计每天每个广告在最近1小时的滑动窗口内的点击趋势（每分钟的点击量）, 每支广告的点击趋势
		
		jssc.start();                                                                                                  //启动
		jssc.awaitTermination();																					   //等待执行
		jssc.close();                                                                                                  //关闭
	}

	/**
	  * @Description: 测试实时分析的高可用性能
	  * @author: wangguoqing
	...
	/**
	  * @Description: 根据黑名单进行过滤，把原始数据中在黑名单中的数据直接给过滤掉
	  * @author: wangguoqing
	  * @date: 2018/12/3 12:14
	  * @Version：1.0.0
	 */
		...
		JavaPairDStream<String, String> filteredAdRealTimeLogDStream = adRealTimeLogDStream.transformToPair(
				new Function<JavaPairRDD<String,String>, JavaPairRDD<String,String>>() {
					...
					public JavaPairRDD<String, String> call(
						...
						IAdBlacklistDAO adBlacklistDAO = DAOFactory.getAdBlacklistDAO();                               //从mysql中查询所有黑名单用户，将其转换为一个rdd
						...
						for(AdBlacklist adBlacklist : adBlacklists) {
							tuples.add(new Tuple2<Long, Boolean>(adBlacklist.getUserid(), true));                      //构建黑名单List->[tuple,tupple,tuple,...]   tuple<userid,boolean>
						}
						
						JavaSparkContext sc = new JavaSparkContext(rdd.context());
						JavaPairRDD<Long, Boolean> blacklistRDD = sc.parallelizePairs(tuples);                         //将list转化成Rdd
						...
						// 将原始日志数据rdd，与黑名单rdd，进行左外连接,如果说原始日志的userid，没有在对应的黑名单中那么就join不到
						// 用inner join，内连接会导致数据丢失
						...
						JavaPairRDD<Long, Tuple2<Tuple2<String, String>, Optional<Boolean>>> filteredRDD = joinedRDD.filter(
								new Function<Tuple2<Long,Tuple2<Tuple2<String,String>,Optional<Boolean>>>, Boolean>() {
									...
									public Boolean call(
										...
										if(optional.isPresent() && optional.get()) {                                   // 如果这个值存在，那么说明原始日志中的userid，join到了某个黑名单用户
											return false;
										}
										return true;
									}
	...
	private static void generateDynamicBlacklist(JavaPairDStream<String, String> filteredAdRealTimeLogDStream) {
		//返回的数据类型：<datekey_userid_adid,1L>
		JavaPairDStream<String, Long> dailyUserAdClickDStream = filteredAdRealTimeLogDStream.mapToPair(
				new PairFunction<Tuple2<String,String>, String, Long>() {
					...
					public Tuple2<String, Long> call(Tuple2<String, String> tuple)
						...
						String key = datekey + "_" + userid + "_" + adid;         //datekey_userid_adid
						return new Tuple2<String, Long>(key, 1L);
					}
				});
		//返回的数据类型：<datekey_userid_adid,nL>      每个时间点每个广告的点击量
...
//				}, 1000);
		...
		// <yyyyMMdd_userid_adid, clickCount>   当天的所有batch中的每个用户对某个广告的点击总次数
			...
			public Void call(JavaPairRDD<String, Long> rdd) throws Exception {
				rdd.foreachPartition(new VoidFunction<Iterator<Tuple2<String,Long>>>() {
					...
					public void call(Iterator<Tuple2<String, Long>> iterator) throws Exception {
						// 对每个分区的数据就去获取一次连接对象，每次都是从连接池中获取，而不是每次都创建，写数据库操作，性能已经提到最高了
	...
	/**
	  * @Description: 计算广告点击流量实时统计,传入的RDD<tuple<value,vlaue2>> ,
	  *         		value2中包含的是真实到的数据信息
	  * @author: wangguoqing
	...
	private static JavaPairDStream<String, Long> calculateRealTimeStat(JavaPairDStream<String, String> filteredAdRealTimeLogDStream) {
		JavaPairDStream<String, Long> mappedDStream = filteredAdRealTimeLogDStream.mapToPair(                          //第一步：DStream ======>tuple<time_province_city_adid,1L>
				new PairFunction<Tuple2<String,String>, String, Long>() {
					private static final long serialVersionUID = 1L;
					@Override
		...
		JavaPairDStream<String, Long> aggregatedDStream =                                                              //第二步：DStream ======>tuple<time_province_city_adid,nL>,对上一步的结果进行累加
				mappedDStream.updateStateByKey(new Function2<List<Long>, Optional<Long>, Optional<Long>>() {           //统计全局的key的状态
					private static final long serialVersionUID = 1L;
					@Override
					public Optional<Long> call(List<Long> values, Optional<Long> optional)
							throws Exception {
						long clickCount = 0L;
						if(optional.isPresent()) { 						                                               // 如果说，之前是存在这个状态的，那么就以之前的状态作为起点，进行值的累加
							clickCount = optional.get();
						}
						for(Long value : values) {                                                                     // values，代表了，batch rdd中，每个key对应的所有的值
							clickCount += value;
						}
	...
	private static void calculateProvinceTop3Ad(
		...
		// adRealTimeStatDStream,每一个batch rdd都代表了最新的全量的每天各省份各城市各广告的点击量
	...
	/**
	  * @Description: 计算最近1小时滑动窗口内的广告点击趋势
	  * @author: wangguoqing
	  * @date: 2018/12/3 12:15
	  * @Version：1.0.0
	 */
	private static void calculateAdClickCountByWindow(
		...
		// aggrRDD-->每次都可以拿到，最近1小时内，各分钟（yyyyMMddHHMM）各广告的点击量, 各广告在最近1小时内，各分钟的点击量
