package com.gsau.sparkproject.test;
...
public class MockData {
   private  static  File file = new File("D:\\WorkSpace\\SparkEdit\\session_data3.txt");
   private  static  PrintStream ps;
    ...
    public static void mock(JavaSparkContext sc,
        ...
        String[] searchKeywords = new String[]{"火锅", "蛋糕", "重庆辣子鸡", "重庆小面",              //搜索的关键词
                "呷哺呷哺", "新辣道鱼火锅", "国贸大厦", "太古商场", "日本料理", "温泉"};
        String date = DateUtils.getTodayDate();
        String[] actions = new String[]{                         //定义四中行为动作
                "search",                                        //搜索
                "click",                                         //点击
                "order",                                         //订单
                "pay"};                                          //支付
        ...
        for (int i = 0; i < 100; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                String sessionid = UUID.randomUUID().toString().replace("-", "");    //生成sessionID
                String baseActionTime = date + " " + random.nextInt(23);                          //行为产生的时间
                Long clickCategoryId = null;                                                              //品类ID
    ...
    /**
     * @Description: 把生成的数据写到本地文件中去
     * @author: wangguoqing
     * @date: 2018/10/29 10:58
     */
    public static void write2LocalFile( String data) {
        ps.append(data + "\n");
    }

    public static void mook_data() {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                ...
                for (int k = 0; k < random.nextInt(100); k++) {
                    ...
                    object.put("searchKeyword", searchKeyword);
                    object.put("clickCategoryId", clickCategoryId);
                    object.put("clickProductId", clickProductId);
                    object.put("orderCategoryId", orderCategoryIds);
                    object.put("orderProductIds", orderProductIds);
                    object.put("payCategoryIds", payCategoryIds);
                    object.put("payProductIds", payProductIds);
                    object.put("id", Long.valueOf(String.valueOf(random.nextInt(10))).toString());
                    rows.add(object);
